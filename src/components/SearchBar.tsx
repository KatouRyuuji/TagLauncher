...
export function SearchBar({ onAddItem, onRefresh, onOpenAbout }: SearchBarProps) {
  ...
  return (
    <header className="px-5 py-3 border-b border-white/[0.06] flex items-center gap-3 bg-[#0a0a0a]">
      {/* 搜索模式切换按钮组 */}
      ...
      <div className="flex-1 relative">
        ...
        <input
          ...
          className="w-full bg-white/[0.04] border border-white/[0.06] rounded-lg pl-9 pr-8 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500/50 focus:bg-white/[0.06] transition-all"
        />
        {/* 清空按钮（有输入内容时显示） */}
        {inputValue && (
          <button
      ...
      {/* 刷新按钮 */}
      <button onClick={onRefresh} className="p-2 text-white/40 hover:text-white/70 transition-colors" title="刷新">
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <button
        onClick={onOpenAbout}
        className="px-3 py-2 bg-white/[0.04] border border-white/[0.06] rounded-lg text-sm text-white/60 hover:bg-white/[0.08] hover:text-white transition-all"
      >
        关于我
      </button>
