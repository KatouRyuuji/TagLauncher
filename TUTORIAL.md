...
3. `searchWithIndex`：先 `parseQuery` 解析表达式（`&&` `||` 空格 `()` `@` `!!`），对每个非严格子项做同义词扩展，再以前缀匹配 / 拼音 / 英文低容错 / 英文缩写弱命中 / 严格匹配逐项求值。英文容错对 ≥3 字符的英文查询做整名前缀编辑距离 ≤1，后段英文词编辑距离恰好为 1；含空格、扩展名或中英文混合的名称同样适用。结果按收藏置顶、同组内强命中先于弱命中输出。
...
- 关键函数：`build_endpoint`（端点归一化，补 `/v1/messages`）、`build_request_body`（显式 `thinking:{type:"disabled"}` 禁用思考——结构化输出任务不需要思考，消除思考模型间歇性无文本响应）、`extract_text`（Anthropic `content[].text` 优先并跳过 thinking，兼容 content 字符串 / OpenAI choices 分段 / HTTP 200 错误体；失败时附响应结构诊断）、`parse_tag_list`（JSON 数组优先，逗号/全角逗号/换行回退）。均带单测（含 mock HTTP 服务器验证瞬态重试）。
...
- 匹配强度 `MatchScore`：2 为强命中（前缀 / 拼音 / 英文容错 / CJK 子串 / 严格），1 为弱命中（英文首字母缩写 / 连写子序列）。
- `searchWithIndex` 按收藏置顶、同组内强命中先于弱命中分桶输出。
- 调整后需手测中文、拼音、英文容错、同义词检索效果。
