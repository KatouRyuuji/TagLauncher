...
- 主题系统:7 大内置配色家族(霜靛 / 藤色 / 柳染 / 水浅葱 / 樱花 / 海军冰蓝 / 铁锈,色值逐值取自 RyuujiDesign 锁定色板),亮 / 暗为独立开关(可跟随系统),窗口栏一键切换;造型语言(纸面 A / 仪表 B)由主题自身决定。另支持自定义 JSON 主题、Mod 主题,导入 / 导出 / 刷新;应用等待主题就绪后再显示主窗口,避免启动闪烁。
- 字体方案:UI 界面 Noto Sans SC、正文 LXGW WenKai、等宽 Cascadia Code,全部 OFL 许可本地打包,开箱即是设计字形。
</details>
...
| [版本对比](./版本对比.md) | 1.0.0 → 1.7.10-beta 代际升级记录 |
